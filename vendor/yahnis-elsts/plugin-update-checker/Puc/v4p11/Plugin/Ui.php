...
if ( !class_exists('Puc_v4p11_Plugin_Ui', false) ):
	...
	class Puc_v4p11_Plugin_Ui {
		...
		public function displayManualCheckResult() {
			if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->updateChecker->slug) ) {	// @codingStandardsIgnoreLine
				$status = strval($_GET['puc_update_check_result']);	// @codingStandardsIgnoreLine
				...
				printf(
					...
					esc_html( apply_filters($this->updateChecker->getUniqueName('manual_check_message'), $message, $status) ),
					esc_html( $details )
				);
			}
		}
