...
if ( !class_exists('Puc_v4p11_Vcs_GitHubApi', false) ):
	...
	class Puc_v4p11_Vcs_GitHubApi extends Puc_v4p11_Vcs_Api {
		...
		protected function api($url, $queryParams = array()) {
			...
			$options = array('timeout' => 10);	// @codingStandardsIgnoreLine
			if ( $this->isAuthenticationEnabled() ) {
			...
			$response = vip_safe_wp_remote_get($url, $options);
		...
		public function addHttpRequestFilter($result) {
			if ( !$this->downloadFilterAdded && $this->isAuthenticationEnabled() ) {
				add_filter('http_request_args', array($this, 'setUpdateDownloadHeaders'), 10, 2);	// @codingStandardsIgnoreLine
