...
<div class="wrap">
    ...
    <form action="<?php echo esc_url( get_admin_url() . 'admin.php?page=dm-credentials&action=save_data' ); ?>" method="post">
        ...
        <table class="form-table" role="presentation">
            ...
            <tr>
                <th scope="row"><label for="api-key"><?php echo esc_html('API Key'); ?> <span class="detail-info">?<span class="tooltip">API key creation on a dailymotion account is subject to your role (editor, admin, owner) in the organization.
If you do not have access to the API key creation page, ask the owner of your organization to create it for you.</span></span></label></th>
                <td>
                    <input name="api_key" type="text" id="api-key" class="regular-text"
                           value="<?php echo (!empty($options)) ? esc_attr( $options['api_key'] ) : '' ?>">
            ...
            <tr>
                <th scope="row"><label for="api-secret"><?php echo esc_html('API Secret'); ?></label></th>
                <td>
                    <input name="api_secret" type="text" id="api-secret" class="regular-text"
                           value="<?php echo (!empty($options)) ? esc_attr( $options['api_secret'] ) : '' ?>">
                </td>
            </tr>
            </tbody>
